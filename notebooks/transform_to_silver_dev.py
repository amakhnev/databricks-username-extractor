...
dbutils.fs.ls('/FileStore/tables/dev/bronze')
...
df_bronze = spark.read.csv('dbfs:/FileStore/tables/dev/bronze/companies.csv', header=True, inferSchema=True)
...
df_companies.write.format('delta').mode('overwrite').save('dbfs:/FileStore/tables/dev/silver/companies')

# Write the SIC codes DataFrame to the silver layer
df_sic_codes.write.format('delta').mode('overwrite').save('dbfs:/FileStore/tables/dev/silver/sic_codes')

# Write the relationship DataFrame to the silver layer
df_relationship.write.format('delta').mode('overwrite').save('dbfs:/FileStore/tables/dev/silver/company_sic_relationship')
